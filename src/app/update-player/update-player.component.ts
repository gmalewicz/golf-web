  ...
  onSubmit() {
    ...
    if (
      ...
      this.alertService.success(
        $localize`:@@updatePlayer-noData:Handicap, password or email must be provided`,
        false,
      );
      return;
    }
    ...
    this.httpService
      ...
      .pipe(
        tap(() => {
          ...
          this.loading = false;
          this.alertService.success(
            $localize`:@@notifications-UpdateMsg:Player data has been updated.`,
            true,
          );
  ...
  onRemove() {
    ...
    dialogRef
        ...
        mergeMap((result: boolean) => {
          if (result) {
            ...
            return firstValueFrom(
              this.httpService.deleteEmail().pipe(map(() => true)),
            );
          }
      ...
      .subscribe((status: boolean) => {
        ...
        if (status === true) {
          this.alertService.success(
            $localize`:@@notifications-DeleteEmailMsg:Email has been deleted.`,
            true,
          );
          this.router.navigate(["/home"]).catch((error) => console.log(error));
        }
