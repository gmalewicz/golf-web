...
export class RoundComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (history.state.data === undefined || this.authenticationService.currentPlayerValue === null) {
      ...
      this.viewOnly = true;
      const isTournamentRound: boolean = history.state.data.tournamentRound === true;
  ...
  private calculateHoleResult(idx: number) {
  ...
  private calculateScoreDiff(pl: Player, idx: number) {
    if (pl.roundDetails!.ninesFull === 0) {
      ...
      const corScore = this.round.scoreCard!.slice(idx * 18, (idx * 18) + 18).map(s => s.corScoreBrutto ?? 0).reduce((p, n) => p + n, 0);
      ...
      pl.roundDetails!.scoreDiff = calculateScoreDifferential(pl.roundDetails!.sr,
                                                            this.round.scoreCard!.slice(idx * 18, (idx * 18) + 18).
                                                              map(s => s.corScoreBrutto ?? 0).reduce((p, n) => p + n, 0),
                                                            pl.roundDetails!.cr,
                                                            false,
                                                            par,
                                                            pl.roundDetails!.whs
