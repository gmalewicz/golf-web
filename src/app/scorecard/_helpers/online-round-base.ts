...
export class OnlineRoundBaseComponent implements OnDestroy, OnInit {
  ...
  public puttSelectorActive = new Array(6)
    .fill(null)
    .map(() => ({ active: false }));
  public penaltySelectorActive = new Array(6)
    .fill(null)
    .map(() => ({ active: false }));
  ...
  ngOnInit(): void {
    ...
    if (
      ...
      this.onlineRoundsSgn().sort(
        (or1, or2) => or1.team - or2.team || or1.player.id - or2.player.id,
      );
      ...
      this.curHoleStrokesSgn.set(
        new Array(this.onlineRoundsSgn().length).fill(0),
      );
      this.curHolePenalties = new Array(this.onlineRoundsSgn().length).fill(0);
      this.strokesSgn.set(
        new Array(18)
          .fill(0)
          .map(() => new Array(this.onlineRoundsSgn().length).fill(0)),
      );
      this.penaltiesSgn.set(
        new Array(18)
          .fill(0)
          .map(() => new Array(this.onlineRoundsSgn().length).fill(0)),
      );
      ...
      this.totalStrokesSgn.set(
        new Array(this.onlineRoundsSgn().length).fill(0),
      );
      this.ballPickedUpSgn.set(
        new Array(this.onlineRoundsSgn().length).fill(false),
      );
  ...
  selectHole(holeIdx: number) {
    ...
    if (
      this.onlineRoundsSgn()[0].tee.teeType === teeTypes.TEE_TYPE_FIRST_9 &&
      holeIdx > 8
    ) {
    ...
    if (
      this.onlineRoundsSgn()[0].tee.teeType === teeTypes.TEE_TYPE_LAST_9 &&
      holeIdx <= 8
    ) {
    ...
    this.curHoleStrokesSgn.set(
      this.curHoleStrokesSgn().map(
        (_s, idx) => this.strokesSgn()[this.curHoleIdxSgn()][idx],
      ),
    );
    // tslint:disable-next-line: variable-name
    this.curHolePutts = this.curHolePutts.map(
      (_s, idx) => this.puttsSgn()[this.curHoleIdxSgn()][idx],
    );
    // tslint:disable-next-line: variable-name
    this.curHolePenalties = this.curHolePenalties.map(
      (_s, idx) => this.penaltiesSgn()[this.curHoleIdxSgn()][idx],
    );
    // in case if stroke is 0 load par instead
    this.curHoleStrokesSgn.set(
      this.curHoleStrokesSgn().map((s, idx) => {
        if (s === 0) {
    ...
    this.puttSelectorActive[this.curHolePutts[this.curPlayerIdxSgn()]] = {
      active: true,
    };
    this.penaltySelectorActive.fill({ active: false });
    this.penaltySelectorActive[this.curHolePenalties[this.curPlayerIdxSgn()]] =
      { active: true };
  }
    ...
    this.dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        ...
        this.scorecardHttpService
          .finalize(this.onlineRoundsSgn()[0].identifier)
          .pipe(
            tap(() => {
              this.loadingFinSgn.set(false);
              this.alertService.success(
                $localize`:@@onLineRndBse-finRndMsg:The round has been successfuly saved`,
                true,
              );
              this.router
                .navigate(["/rounds"])
  ...
  onDelete() {
    ...
    this.dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        ...
        this.scorecardHttpService
          .deleteOnlineRound(this.onlineRoundsSgn()[0].identifier)
          .pipe(
            tap(() => {
              this.loadingDelSgn.set(false);
              this.alertService.success(
                $localize`:@@onLineRndBse-delRndMsg:This score card has been successfully deleted`,
                false,
              );
              this.router
                .navigate(["/home"])
  ...
  addScore() {
    ...
    if (
      this.curHoleStrokesSgn()[this.curPlayerIdxSgn()] !==
        this.strokesSgn()[this.curHoleIdxSgn()][this.curPlayerIdxSgn()] ||
      this.curHolePutts[this.curPlayerIdxSgn()] !==
        this.puttsSgn()[this.curHoleIdxSgn()][this.curPlayerIdxSgn()] ||
      this.curHolePenalties[this.curPlayerIdxSgn()] !==
        this.penaltiesSgn()[this.curHoleIdxSgn()][this.curPlayerIdxSgn()]
    ) {
      ...
      if (
        (this.curHoleStrokesSgn()[this.curPlayerIdxSgn()] !==
          ...
          this.curHolePutts[this.curPlayerIdxSgn()] !==
            this.puttsSgn()[this.curHoleIdxSgn()][this.curPlayerIdxSgn()] ||
          this.curHolePenalties[this.curPlayerIdxSgn()] !==
            this.penaltiesSgn()[this.curHoleIdxSgn()][
              this.curPlayerIdxSgn()
            ]) &&
  ...
  private updateTotals() {
    ...
    this.totalStrokesSgn()[this.curPlayerIdxSgn()] -=
      this.strokesSgn()[this.curHoleIdxSgn()][this.curPlayerIdxSgn()];
    this.totalStrokesSgn()[this.curPlayerIdxSgn()] +=
      this.curHoleStrokesSgn()[this.curPlayerIdxSgn()];
  ...
  private sendMessage(onlineScoreCard: OnlineScoreCard) {
    ...
    this.rxStompService.publish({
      destination: "/app/hole",
      headers: { receipt: receiptId },
      body: JSON.stringify(onlineScoreCard),
    });
  ...
  private setBallPickUp() {
    for (let idx = 0; idx < 18; idx++) {
      if (
        this.strokesSgn()[idx][this.curPlayerIdxSgn()] === ballPickedUpStrokes
      ) {
  ...
  loadScoreCards() {
    ...
    for (let i = 0; i < this.onlineRoundsSgn().length; i++) {
      ...
      this.calculateHCP(i);
      calls[i] = this.scorecardHttpService.getOnlineScoreCard(
        this.onlineRoundsSgn()[i].id,
      );
    }
    ...
    combineLatest(calls)
      .pipe(
        tap((onlineScoreCards: OnlineScoreCard[][]) => {
          ...
          this.curHoleStrokesSgn.set(
            this.curHoleStrokesSgn().map((s, idx) => {
              ...
              if (s === 0) {
                ...
                this.curHolePutts[idx] =
                  this.puttsSgn()[this.curHoleIdxSgn()][idx];
                this.curHolePenalties[idx] =
                  this.penaltiesSgn()[this.curHoleIdxSgn()][idx];

                if (idx === 0) {
                  this.puttSelectorActive[2] = { active: false };
                  this.puttSelectorActive[this.curHolePutts[0]] = {
                    active: true,
                  };
                  this.penaltySelectorActive[0] = { active: false };
                  this.penaltySelectorActive[this.curHolePenalties[0]] = {
                    active: true,
                  };
                }
              }
  ...
  private initCurHoleIdx(onlineScoreCards: OnlineScoreCard[][]) {
    ...
    if (
      ...
      this.curHoleIdxSgn.set(
        Math.max(...onlineScoreCards[0].map((scoreCard) => scoreCard.hole)) - 1,
      );
    }
  }
  ...
  private checkIfRoundCompleted() {
    ...
    if (
      (this.onlineRoundsSgn()[0].tee.teeType === teeTypes.TEE_TYPE_FIRST_9 &&
        this.curHoleIdxSgn() === 8) ||
      this.curHoleIdxSgn() === 17
    ) {
  ...
  private handleDocumentVisibilityChanges(): void {
    ...
    this.subscriptions.push(
      this.visibilityChangeEvent.subscribe(() => {
  ...
  onInfo() {
    if (
      this.onlineRoundsSgn()[0].format !== Format.MATCH_PLAY &&
      this.onlineRoundsSgn()[0].courseHCP == undefined
    ) {
      this.onlineRoundsSgn().forEach((onlineRound) => {
        onlineRound.courseHCP = calculateRoundedCourseHCP(
          onlineRound.tee.teeType,
          onlineRound.player.whs,
          onlineRound.tee.sr,
          onlineRound.tee.cr,
          getPlayedCoursePar(
            this.courseSgn().holes,
            onlineRound.tee.teeType,
            this.courseSgn().par,
          ),
        );
    ...
    this.router
      .navigate(["myScorecard/onlineScoreCardInfo"], {
        state: { onlineRounds: this.onlineRoundsSgn() },
      })
      .catch((error) => console.log(error));
  }
}
